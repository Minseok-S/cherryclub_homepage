...
export async function PATCH(
  ...
  try {
    ...
    await connection.query(
      "UPDATE notice_comments SET content = ?, updated_at = NOW() WHERE id = ?",
      [content, id]
    );
    ...
    const [updatedRows] = await connection.query(
      `SELECT 
        c.id, c.notice_id, c.content, c.parent_id,
...
export async function DELETE(
  ...
  try {
    ...
    // 댓글 존재 여부 및 작성자 확인
    const [commentRows] = await connection.query(
      "SELECT id, author_id, parent_id FROM notice_comments WHERE id = ?",
      [id]
    );
    ...
    if (parentId === null) {
      const [repliesRows] = await connection.query(
        "SELECT COUNT(*) AS reply_count FROM notice_comments WHERE parent_id = ?",
        [id]
      );
      ...
      if (replyCount > 0) {
        await connection.query(
          "UPDATE notice_comments SET content = '삭제된 댓글입니다.', updated_at = NOW() WHERE id = ?",
          [id]
        );
    ...
    // 댓글 삭제
    await connection.query("DELETE FROM notice_comments WHERE id = ?", [id]);
